...
const S = {
  ...
  headerTitle: { flex: 1, fontWeight: 600, fontSize: 14, color: 'var(--color-text-primary)' },
  headerSub:   { fontSize: 11, color: 'var(--color-text-tertiary)' },
  messages: {
  ...
  confirmQuestion: { color: '#92400e', marginBottom: 10, fontWeight: 500 },
  ...
  footer: {
    padding: '10px 14px', borderTop: '1px solid var(--color-border-tertiary)',
    background: 'var(--color-background-secondary)',
...
// La réponse de l'agent arrive sous la forme :
//   data.response = '{"type":"confirmation_required","question":"...","confirm_text":"...","cancel_text":"..."}'
// Parfois avec guillemets échappés (\") quand sérialisé deux fois.

function parseConfirmation(content) {
  if (!content || typeof content !== 'string') return null;

  // Cas 1 — contenu est directement le JSON
  try {
    const parsed = JSON.parse(content);
    if (parsed && parsed.type === 'confirmation_required') return parsed;
  } catch (_) {}

  // Cas 2 — guillemets échappés (double sérialisation Python json.dumps)
  // Ex : "{\"type\": \"confirmation_required\", ...}"
  try {
    const unescaped = content
      .replace(/^"/, '').replace(/"$/, '')  // enlever guillemets extérieurs
      .replace(/\\"/g, '"')                  // désescaper \"
      .replace(/\\n/g, '\n');                // désescaper \n
    const parsed = JSON.parse(unescaped);
    if (parsed && parsed.type === 'confirmation_required') return parsed;
  } catch (_) {}

  // Cas 3 — JSON embarqué quelque part dans une string plus longue
  // Cherche { ... "type": "confirmation_required" ... } avec un regex large
  try {
    const match = content.match(/\{[\s\S]*?"type"\s*:\s*"confirmation_required"[\s\S]*?\}/);
    if (match) {
  ...
  // Cas 4 — texte simple "CONFIRMATION REQUISE: ..."
...
export default function AiAgent() {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: 'Impossible de joindre le service agent.',
      }]);
    } finally {
      setStreaming(false);
      setStreamingText('');
  ...
  const renderMessage = (msg, i) => {
    if (msg.role !== 'assistant') {
      return <div key={i} style={S.bubble('user')}>{msg.content}</div>;
    }
  ...
  const TypingIndicator = () => (
    <div style={{ ...S.bubble('assistant'), padding: '10px 14px' }}>
      <style>{`
        ...
        .d2{animation:bounce 1.2s infinite .2s}
        .d3{animation:bounce 1.2s infinite .4s}
      `}</style>
        ...
        <div style={S.panel}>
          <div style={S.header}>
            ...
            <Tooltip title="Effacer la conversation">
              <Button size="small" icon={<ClearOutlined/>} onClick={clearChat} type="text" style={{ color: 'var(--color-text-tertiary)' }}/>
            </Tooltip>
            <Button size="small" icon={<CloseOutlined/>} onClick={() => setOpen(false)} type="text" style={{ color: 'var(--color-text-tertiary)' }}/>
          </div>
