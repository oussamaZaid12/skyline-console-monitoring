...
export default class VmRanking extends Component {
  ...
  fetchData = async () => {
    try {
      ...
      // Construit la liste des instances uniques (pour AlertsModal)
      const allInstances = [
        ...(data.by_cpu || []),
        ...(data.by_memory || []),
        ...(data.by_network || []),
  ...
  makeColumns = (mainKey, mainLabel, mainColor, mainRender, extraCols = []) => [
    {
      ...
      key: 'rank',
      width: 48,
      render: (_, __, i) => <RankBadge index={i} />,
    },
    {
      title: t('Instance'),
      key: 'instance',
      render: (_, row) =>
        row.uuid ? (
          <a
            href={`/compute/instance/detail/${row.uuid}?tab=monitoring`}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <MiniBar
              percent={v}
              color={
                v > 80 ? COLOR.danger : v > 50 ? COLOR.warning : COLOR.primary
              }
            />
          </div>
          <span
      ...
      [
        {
          ...
          width: 110,
          render: (v) => (
            <span style={{ color: COLOR.textBody }}>{v.toFixed(0)} MB</span>
          ),
        },
        {
          title: t('Network RX'),
          dataIndex: 'network_rx_kbps',
          key: 'network_rx_kbps',
          width: 120,
          render: (v) => (
            <span style={{ color: COLOR.textBody }}>{v.toFixed(2)} KB/s</span>
          ),
        },
      ]
    );
  ...
  memoryColumns = () =>
    this.makeColumns(
      ...
      (v) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <MiniBar percent={(v / 4096) * 100} color={COLOR.success} />
          </div>
          <span
            style={{
              width: 60,
      ...
      [
        {
          ...
          width: 80,
          render: (v) => (
            <span style={{ color: COLOR.textBody }}>{v.toFixed(1)}%</span>
          ),
        },
      ]
    );
  ...
  networkColumns = () =>
    this.makeColumns(
      ...
      (v) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <MiniBar percent={Math.min(v, 100)} color={COLOR.purple} />
          </div>
          <span
            style={{
              width: 72,
  ...
  render() {
    ...
    const statCards = [
      {
        label: t('Top CPU Consumer'),
        icon: <RiseOutlined />,
        value: topCpu ? topCpu.cpu_percent : 0,
        suffix: '%',
        precision: 1,
        instance: topCpu?.name || '-',
        color: COLOR.danger,
      },
      {
        label: t('Top Memory Consumer'),
        icon: <DatabaseOutlined />,
        value: topMem ? topMem.memory_mb : 0,
        suffix: 'MB',
        precision: 0,
        instance: topMem?.name || '-',
        color: COLOR.primary,
      },
      {
        label: t('Top Network Consumer'),
        icon: <WifiOutlined />,
        value: topNet ? topNet.network_rx_kbps : 0,
        suffix: 'KB/s',
        precision: 2,
        instance: topNet?.name || '-',
        color: COLOR.purple,
      },
    ];
          ...
          {statCards.map((s, i) => (
            <Col span={8} key={i}>
              ...
              >
                ...
                <div style={{ fontSize: 12, color: COLOR.textCaption }}>
                  {s.instance}
                </div>
              </Card>
            </Col>
        ...
        >
          ...
          >
            <TabPane
              tab={
                <span style={{ fontSize: 13 }}>
                  <RiseOutlined /> {t('By CPU')}
                </span>
              }
              key="cpu"
            >
              <Table
                dataSource={data.by_cpu}
                columns={this.cpuColumns()}
                rowKey="domain"
                pagination={false}
                size="middle"
              />
            ...
            <TabPane
              tab={
                <span style={{ fontSize: 13 }}>
                  <DatabaseOutlined /> {t('By Memory')}
                </span>
              }
              key="memory"
            >
              <Table
                dataSource={data.by_memory}
            ...
            <TabPane
              tab={
                <span style={{ fontSize: 13 }}>
                  <WifiOutlined /> {t('By Network')}
                </span>
              }
              key="network"
            >
              <Table
                dataSource={data.by_network}
