...
const MiniBar = ({ percent, color }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
    <div style={{ flex: 1, height: 6, background: COLOR.bg, borderRadius: 3, overflow: 'hidden', minWidth: 80 }}>
      <div style={{ width: `${Math.min(percent, 100)}%`, height: '100%', background: color, borderRadius: 3, transition: 'width 0.4s ease' }} />
    </div>
  </div>
);
...
const RankBadge = ({ index }) => {
  ...
  return (
    <div style={{ width: 22, height: 22, borderRadius: '50%', background: colors[index] || COLOR.textCaption, color: '#fff', fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {index + 1}
    </div>
  );
};
...
export default class VmRanking extends Component {
  ...
  makeColumns = (mainKey, mainLabel, mainColor, mainRender, extraCols = []) => [
    ...
    { title: mainLabel, dataIndex: mainKey, key: mainKey, width: 260, sorter: (a, b) => a[mainKey] - b[mainKey], render: mainRender },
    ...extraCols,
  ];

  cpuColumns = () =>
    this.makeColumns('cpu_percent', t('CPU Usage'), COLOR.primary,
      (v) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1 }}><MiniBar percent={v} color={v > 80 ? COLOR.danger : v > 50 ? COLOR.warning : COLOR.primary} /></div>
  ...
  memoryColumns = () =>
    this.makeColumns('memory_mb', t('Memory Usage'), COLOR.success,
      (v) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
  ...
  networkColumns = () =>
    this.makeColumns('network_rx_kbps', t('Network RX'), COLOR.purple,
      (v) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
  ...
  render() {
    ...
    // AiAgent toujours rendu — même en cas d'erreur ou de chargement
    if (loading && !data)
      return (
        <>
    ...
    if (error)
      return (
        <>
          <Alert
            message={t('Error Loading Metrics')}
            description={error}
            type="error"
            showIcon
            style={{ margin: 24 }}
    ...
    return (
      <div style={{ padding: '20px 24px', background: COLOR.bg, minHeight: '100%' }}>
        ...
        {/* Tables */}
        <Card bordered={false} bodyStyle={{ padding: 0 }} style={{ background: COLOR.bgCard, borderRadius: 4, boxShadow: '0 2px 6px rgba(36,46,66,0.06)' }}>
          <Tabs defaultActiveKey="cpu" style={{ padding: '0 20px' }} tabBarStyle={{ marginBottom: 0, borderBottom: `1px solid ${COLOR.border}` }}>
        ...
        {/* Assistant AIOps — toujours visible */}
        <AiAgent />
