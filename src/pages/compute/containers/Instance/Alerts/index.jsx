...
import {
  Table, Card, Row, Col, Button, Form, Switch, Tag,
  notification, Select, InputNumber, Input, Empty,
} from 'antd';
import {
  BellOutlined, PlusOutlined, DeleteOutlined,
  EditOutlined, CheckCircleOutlined, ReloadOutlined,
  WarningOutlined,
...
export default class Alerts extends Component {
  ...
  fetchAll = async (showToast) => {
    ...
    this.setState({ loading: false, lastUpdate: new Date().toLocaleTimeString() });
  };

  openCreateForm = () => {
    this.setState({ editingRule: null, notifyEmail: false, notifyWebhook: false, showForm: true }, () => {
  ...
  openEditForm = (rule) => {
    this.setState({
      ...
      if (this.formRef.current) this.formRef.current.setFieldsValue(rule);
    });
  };

  closeForm = () => {
    this.setState({ showForm: false, editingRule: null });
  };

  handleSave = async () => {
    const { instances, editingRule } = this.state;
    try {
      ...
      this.setState({ showForm: false, editingRule: null, notifyEmail: false, notifyWebhook: false });
      this.fetchRules();
      ...
      notification.error({ message: t('Error'), description: String(e), duration: 5 });
    }
  };
  ...
  handleResolve = async (eventId) => {
    await this.apiFetch(`/alerts/events/${eventId}/resolve`, { method: 'POST' });
    this.fetchHistory();
  ...
  rulesColumns = () => [
    ...
    {
      title: t('Condition'), key: 'cond',
      render: (_, r) => `${r.operator === 'gt' ? '>' : '<'} ${r.threshold}`,
    },
    {
      title: t('Notifications'), key: 'notifs',
      render: (_, r) => (
        <span>
    ...
    {
      title: t('Active'), key: 'active', width: 90,
      render: (_, r) => (
        <Switch checked={r.is_active} onChange={() => this.handleToggle(r.id)} />
      ),
    },
    {
      ...
      render: (_, r) => (
        <span style={{ display: 'flex', gap: 8 }}>
          <Button size="small" icon={<EditOutlined />} onClick={() => this.openEditForm(r)} />
          <Button size="small" icon={<DeleteOutlined />} danger onClick={() => this.handleDelete(r.id)} />
        </span>
      ),
    },
  ];
  ...
  historyColumns = () => [
    ...
    {
      title: t('Triggered at'), dataIndex: 'triggered_at', key: 'triggered_at',
    ...
    },
    {
      title: '', key: 'resolve',
      render: (_, r) => (!r.is_resolved ? (
        <Button size="small" icon={<CheckCircleOutlined />} onClick={() => this.handleResolve(r.id)}>
          {t('Resolve')}
        </Button>
      ) : null),
    },
  ...
  activeColumns = () => [
    ...
    { title: t('Metric'), dataIndex: 'metric', key: 'metric', render: this.metricLabel },
    {
      ...
      render: (_, r) => (
        <span style={{ color: COLOR.danger, fontWeight: 600 }}>
          {r.value} <span style={{ color: COLOR.textCaption, fontWeight: 400 }}>/ {r.threshold}</span>
        </span>
      ),
    },
    {
    ...
    },
    {
      title: '', key: 'resolve',
      render: (_, r) => (
        <Button size="small" type="primary" ghost icon={<CheckCircleOutlined />} onClick={() => this.handleResolve(r.id)}>
          {t('Resolve')}
        </Button>
      ),
    },
  ...
  render() {
    const {
      loading, rules, history, activeAlerts, activeCount,
      showForm, editingRule, notifyEmail, notifyWebhook, lastUpdate, instances,
    } = this.state;
    ...
    const statCards = [
      {
        label: t('Active Alerts'), icon: <WarningOutlined />, value: activeCount,
        color: activeCount > 0 ? COLOR.danger : COLOR.success,
      },
      {
        label: t('Alert Rules'), icon: <BellOutlined />, value: rules.length,
        color: COLOR.primary,
      },
      {
        label: t('Active Rules'), icon: <CheckCircleOutlined />, value: activeRulesCount,
        color: COLOR.purple,
      },
    ];
    ...
    return (
      <div style={{ padding: '20px 24px', background: COLOR.bg, minHeight: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 600, color: COLOR.textTitle }}>{t('Alerts')}</div>
            <div style={{ width: 32, height: 2, background: COLOR.primary, marginTop: 4, borderRadius: 1 }} />
          </div>
          <div style={{ fontSize: 12, color: COLOR.textCaption, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }} onClick={() => this.fetchAll(false)}>
          ...
          {statCards.map((s, i) => (
            <Col span={8} key={i}>
              ...
              >
                ...
                <div style={{ fontSize: 26, fontWeight: 600, color: s.color, lineHeight: 1.2 }}>
                  {s.value}
                </div>
              </Card>
            </Col>
        ...
        {activeAlerts.length > 0 && (
          <Card
            title={<span><WarningOutlined style={{ color: COLOR.danger, marginRight: 8 }} />{t('Active Alerts')}</span>}
            bordered={false}
            bodyStyle={{ padding: 0 }}
            style={{ marginBottom: 16, background: COLOR.bgCard, borderRadius: 4, boxShadow: '0 2px 6px rgba(36,46,66,0.06)' }}
          >
            <Table
        ...
        >
          {showForm && (
            ...
            >
              <Form ref={this.formRef} layout="vertical" onFinish={this.handleSave}>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 1fr 1fr', gap: 12 }}>
                  ...
                  <Form.Item name="instance_id" label={t('Target Instance')} initialValue="all" rules={[{ required: true }]}>
                    <Select>
                  ...
                  <Form.Item name="metric" label={t('Metric')} initialValue="cpu" rules={[{ required: true }]}>
                    <Select>
                      {METRICS.map((m) => <Option key={m.value} value={m.value}>{m.label}</Option>)}
                    </Select>
                  </Form.Item>
                  <Form.Item name="operator" label={t('Condition')} initialValue="gt">
                    <Select>
                      <Option value="gt">&gt;</Option>
                      <Option value="lt">&lt;</Option>
                    </Select>
                  </Form.Item>
                  <Form.Item name="threshold" label={t('Threshold')} rules={[{ required: true, message: t('Required') }]}>
                    <InputNumber style={{ width: '100%' }} placeholder="80" />
                  </Form.Item>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginTop: 4 }}>
                  <Form.Item name="notify_ui" label="UI" valuePropName="checked" initialValue>
                    <Switch />
                  </Form.Item>
                  <Form.Item name="notify_email" label="Email" valuePropName="checked" initialValue={false}>
                    <Switch onChange={(v) => this.setState({ notifyEmail: v })} />
                  </Form.Item>
                  <Form.Item name="notify_webhook" label="Webhook" valuePropName="checked" initialValue={false}>
                    <Switch onChange={(v) => this.setState({ notifyWebhook: v })} />
                  </Form.Item>
                </div>

                {notifyEmail && (
                  <Form.Item name="email_address" label={t('Notification email')}
                ...
                {notifyWebhook && (
                  <Form.Item name="webhook_url" label={t('Webhook URL (Slack/Teams)')}>
          ...
          <Table
            ...
            size="middle"
            pagination={false}
            locale={{ emptyText: <Empty description={t('No rules yet. Click "New Rule" to create one.')} /> }}
          />
        </Card>
        ...
        >
          <Table
            ...
            locale={{ emptyText: <Empty description={t('No alerts triggered yet.')} /> }}
          />
        </Card>
